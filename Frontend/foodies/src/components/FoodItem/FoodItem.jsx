...
const FoodItem = ({ name, description, imageUrl, price, id }) => {
    const {increaseQnt,decreaseQnt,quantities} = useContext(StoreContext)
    return (
                ...
                <div className="card-footer d-flex justify-content-between bg-light">
                    ...
                    {quantities[id]>0 ?(
                        <div className="d-flex alifn-items-center gap-2">
                            <button className="btn btn-danger btn-sm" onClick={()=>decreaseQnt(id)}>
                                <i className='bi bi-dash-circle'></i>
                            </button>
                            <span className="fw-bold">{quantities[id]}</span>
                            <button className="btn btn-success btn-sm" onClick={()=>increaseQnt(id)}>
                                <i className='bi bi-plus-circle'></i>
                            </button>
                        </div>
                    ):(
                        <button className="btn btn-primary btn-sm" onClick={()=>increaseQnt(id)}>
                            <i className='bi bi-plus-circle'></i>
                        </button>
                    )}
                </div>
